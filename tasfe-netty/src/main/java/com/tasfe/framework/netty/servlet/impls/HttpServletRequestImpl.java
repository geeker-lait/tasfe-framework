...
import com.tasfe.framework.netty.dcoder.HttpPostStandardRequestDecoderSF;
...
public class HttpServletRequestImpl implements HttpServletRequest {
    ...
    public HttpServletRequestImpl(HttpRequest request, FilterChainImpl chain) {
        ...
        if (HttpMethod.POST.name().equalsIgnoreCase(request.method().name())) {
            if (request.headers().contains(HttpHeaderNames.CONTENT_TYPE) && request.headers().get(HttpHeaderNames.CONTENT_TYPE).contains(HttpHeaderValues.APPLICATION_JSON)) {
                postRequestDecoder = new HttpPostStandardRequestDecoderSF(new DefaultHttpDataFactory(false), request, StandardCharsets.UTF_8);
            } else {
                postRequestDecoder = new HttpPostStandardRequestDecoder(new DefaultHttpDataFactory(), request);
            }
        }
